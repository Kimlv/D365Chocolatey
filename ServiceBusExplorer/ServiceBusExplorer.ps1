...
# Name of the package on chocolatey.org
$PackageName = "servicebusexplorer"
# Substitute the working folder and .nuspec file names
$DestinationDirectory = "$(BUILD.SOURCESDIRECTORY)\ServiceBusExplorer\"
$NuSpecPath = ($DestinationDirectory + "ServiceBusExplorer.nuspec")


# Check current Chocolatey version
Write-Host "Checking current version on Chocolatey..."
$packages = Find-Package -Name $PackageName -ProviderName NuGet -Source "chocolatey.org"
$NuGetVersion = [Version]$packages[0].Version
Write-Host "Chocolatey version: "$NuGetVersion
...
# Download file
Write-Host "Downloading $PackageName $NuGetVersion from Chocolatey..."
$webClient = New-Object System.Net.WebClient
# .zip file name needs to match name in chocolateyInstall.ps1
$webClient.DownloadFile("https://chocolatey.org/api/v2/package/$PackageName/$NuGetVersion", ($DestinationDirectory + "ToolPackage.zip"))
...
# push "$(BUILD.SOURCESDIRECTORY)\ServiceBusExplorer\*.nupkg" -ApiKey "$(PackagesReadWrite)" -Source "$(FeedUrl)"
...
#$NuSpecPath = "$(BUILD.SOURCESDIRECTORY)\ServiceBusExplorer\ServiceBusExplorer.nuspec"
#
